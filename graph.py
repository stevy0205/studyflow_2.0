...
Based on the flowchart for a method/tool coaching chatbot.
"""
...
from nodes.session import (
    show_method_detail_node,
    give_start_impulse_node,
    request_feedback_node,
    wait_for_input_node,
)
...
def _after_coach_explain(state: CoachState) -> str:
    """
    Route after coach explanation depending on session phase.

    FIX:
    - previously checked `method_chosen` but routers set `chosen_method`
    """
    if state.get("session_active"):
        return "wait_for_input"
    if state.get("chosen_method"):  # <-- FIXED
        return "show_method_detail"
...
def build_graph():
    ...
    # ── After displaying methods: user actions ────────────────────────────────
    builder.add_conditional_edges("display_methods", route_user_action, {
        "question": "coach_explain",
        "choose_method": "show_method_detail",
    })

    # IMPORTANT CHANGE FOR HTTP CHAT:
    # Stop after producing one AI answer, otherwise graph can loop on the same HumanMessage.
    #
    # If coach_explain produces an AI reply, END immediately.
    builder.add_edge("coach_explain", END)

    # Likewise, if show_method_detail produces an AI reply, END immediately.
    builder.add_edge("show_method_detail", END)

    # If you want the old multi-step behavior, remove the two END edges above
    # and instead implement "message consumption" / interrupts.

    # ── Session flow ───────────────────────────────────────────────────────────
    # These parts will be reached in later user turns (next HTTP calls)
    ...
    builder.add_conditional_edges("wait_for_input", route_input_type, {
        ...
        "other": END,  # stop instead of looping forever in one invoke
    })

    # Feedback categorization routing node
    builder.add_node("route_feedback", lambda state: state)  # pass-through
    ...
    # ── Feedback nodes → next action ───────────────────────────────────────────
    for fb_node in [
        "feedback_positive",
        "feedback_partial_positive",
        "feedback_neutral",
        "feedback_partial_negative",
        "feedback_negative",
    ]:
    ...
    # If you ever re-enable multi-step coach_explain routing, use this conditional:
